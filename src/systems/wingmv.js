...
/** DONDE NACE y HACIA DONDE SE VA, por lado de entrada.
 *
 *  Los tres lados son los del plan: por izquierda, por derecha y **desde atras** — este ultimo
 *  nace CERCA de la camara (z chico, o sea grande en pantalla) y se aleja hacia adelante, que es
 *  como se ve a alguien que te sobrepasa. Los dos laterales nacen FUERA del carril del jugador
 *  (`FLY_X`), y por eso `movesSystem` no les aplica los topes: un actor no juega en el carril.  */
function entrada(lado) {
  if (lado === 'atras') return { x: plane.x - 6, y: plane.y - 2, z: WINGMV.Z0, salX: 0, salZ: 1 };
  const s = lado === 'der' ? 1 : -1;
  return { x: plane.x + s * WINGMV.X0, y: plane.y + WINGMV.DY, z: WINGMV.Z, salX: s, salZ: 0 };
}

/** Mete un actor que vuela `id` y se va. `lado` = 'izq' | 'der' | 'atras'.
 *
 *  `dir` es el sentido de la maniobra; por omision **entra desde un lado y la vuela hacia el
 *  otro**, que es lo que hace que la figura quede EN ESCENA en vez de salirse por donde entro.
...
export function entra(id, lado, o) {
  ...
  o = o || {};
  const e = entrada(lado === 'der' || lado === 'atras' ? lado : 'izq');
  ...
  const mx = o.x === undefined ? plane.x + (e.salX ? -e.salX * WINGMV.ENC : 0) : o.x;
  const my = o.y === undefined ? Math.max(8, plane.y + WINGMV.DY) : o.y;
  const a = {
    id, lado, fase: 'entra', t: 0, vida: 0,
    ...
    est: {
      mv: null, mvT: 0, mvDir: o.dir === undefined ? (e.salX || 1) * -1 : o.dir,
      mvY0: my, mvTgt: o.tgt || my + 20, mvRoll: 0, mvSteep: 0, mvSeed: 0,
      rollT: 0, rollCd: 0, spd: WINGMV.SPD, scrapeT: 0, detection: 0,
    },
...
if (typeof window !== 'undefined') window.__mvactor = (id, lado, dir) => {
  const a = entra(id || 'barrel', lado || 'izq', dir === undefined ? undefined : { dir: +dir });
