...
| [PLAN_TEATRO_AEREO.md](sistemas/PLAN_TEATRO_AEREO.md) | 🟥 **EL TEATRO AÉREO**: que el compañero pelee —le tiran, esquiva con una pirueta, derriba— **sin que nada de eso pueda tocar al jugador**. La regla de oro (el teatro no está en las cinco listas de `world.js`, así que no hay código de daño que apagar), el lugar inaccesible que ya existe (la profundidad: el jugador vuela SIEMPRE a `PZ`), la esquivada sin dados (el tiro apunta al punto que el Fiel acaba de vacar) y fases TA0–TA4 | ✅ **TA0–TA3 implementadas (4/9)** — el reparto y su valla medida atómicamente, la esquivada con separación **acotada por construcción** (perpendicular común fuga↔línea de fuego), el derribo con impacto barrido, y la escena entera como **un verbo** del director (`teatro:`). `npm run maniobras` §4–§6 · TA4 es contenido · la perilla de daño real sigue apagada |
...
| [MECANICAS_LLEGADA.md](sistemas/MECANICAS_LLEGADA.md) | 🟥 **la llegada al blanco como listado de mecánicas** (para reemplazar o acompañar al PULSO): relevamiento de lo existente (§1), las restricciones (la vuelta es el clímax → la llegada dura 10–40 s, sin cambio de estado si se puede), y **~25 opciones en siete familias** — la altura decide (banda de suelta, filo, racha, salto, sapito), puntería con el mundo (carril de eslora, callar la AA, la sombra), presión (la cola, columnas, Sea Cat, buque-barrera), decisión (una o dos, zona por radio, no soltar), tiempo (MOMENTUM del jugador), variantes del PULSO, especiales por misión. `llegada:` como dato en capas (§4), la pieza que destraba todo = la suelta en el pasillo (§5), plan B (§6), cómo probar hoy desde PRUEBAS (§7) | 🟨 **decisión provisoria (14/9, §0): se mantiene EL PULSO** como ataque, alimentado por las dos situaciones de llegada (te vieron → escalera largo/medio/corto + pulso rápido · no te vieron → pulso lento y zona brava). Gate: si el corte a la cabina sigue molestando con contexto, se construye A1 |
...
| [PLAN_PASILLO_ZIGZAG.md](sistemas/PLAN_PASILLO_ZIGZAG.md) | 🟥 **EL CALLEJON DE LAS BOMBAS**: laderas a los costados del PASILLO con **puntas de tierra que se meten adentro y hay que esquivar**, con la cámara completamente quieta. El zigzag lo hace el jugador, no la cámara. Chocar la ladera mata; una punta por banda y de un solo lado, así el callejón no se puede cerrar por construcción. Incluye el carril curvo (riel estilo OutRun) **construido, probado y apagado por dato** (`amp: 0`), para el día que se quiera un callejón que además doble. El callejón empieza pasados 700 m (nunca sobre el despegue), la ladera tiene **meseta de tierra arriba** y se puede **pasar por encima** pagando radar. **La tierra puede ser de UN SOLO LADO** (`paredes.lado`: ambos/izq/der) — costa o acantilado en vez de callejón. Y **BARRERAS** que cierran el pasillo de lado a lado (`paredes.barreras`): **cuatro pieles sobre el mismo objeto** — collado de ROCA (por arriba), PUENTE de vigas, viaducto de MADERA y TENDIDO de alta tensión — la única excepción a la garantía de paso, y a propósito: cambia de eje. Filas PASILLO EN ZIGZAG y BARRERAS (NINGUNA/POCAS/MUCHAS) en OPCIONES · `?zigzag=1..4` · sondas `__zz*` · fixture `npm run zigzag` | 🟡 **Z0–Z3 construidas (4/9/2026)** · `check` verde · `feel` idéntico · **114 divergencias en §10** (dieciocho playtests le cambiaron la tesis al ítem). **Z4, Z5 y Z6 hechas: m5 vuela su callejón** (boca, callejón con antiaéreos, y la bahía abriéndose sobre el ARDENT), cero código — tres líneas de dato) **con antiaéreos disparando desde las laderas en ráfaga** y la radio de la boca; los cerros **recortan** lo que tienen delante (el objeto asoma cortado por el filo, no aparece o desaparece). El TELÓN de tierra del horizonte se construyó y se SACÓ en el playtest — el fondo va a ser una imagen. La NIEBLA ya no trata al cerro como cielo: adentro del cañón se abre hacia arriba en vez de taparlo con una chapa plana. Entradas nuevas en PRUEBAS |
...
| [PLAN_MEJORAS_3D.md](sistemas/PLAN_MEJORAS_3D.md) *(en sistemas/)* | 🟥 **los planes ejecutables de los referentes**: P1 agua cartoon · P3 duotono · P6 bruma · P2 bandada · P4 terreno de bahía · P5 nombres — con el MAPA de qué va secuencial (carril 3D: una sesión por vez en `three-arena`) y qué en paralelo, los gateados (P7, prototipo `?p3d`) y **un prompt pegable por plan para Opus medio** | **listo para delegar — arrancar por P1** |
| [ANALISIS_REFERENTES_3D.md](proyecto/ANALISIS_REFERENTES_3D.md) | 🟥 **tres juegos three.js jugados y analizados** (Pigeon: A Love Story · Battle Typer · GliderVR) + la respuesta a "¿por qué RASANTE no puede?" (puede: mismo stack, el gap es dirección de arte del 3D) + **6 propuestas**: agua cartoon (revisa PLAN_AGUA F8 con evidencia), la bandada, el duotono para T7, el TERRENO 3D barato (bahías con lomas para los clímax), nombres en el mundo, bruma en capas | **análisis + propuestas — P1 delegable ya** |
