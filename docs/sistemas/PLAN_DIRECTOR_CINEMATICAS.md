...
| **C1 · Los actores** | verbo `actor` con trayectorias reusando el relevo y el líder de PERSECUCIÓN (los Fieles con indicativo, el Harrier de LA COLA, la Chancha). **El relevo migrado** como segundo timeline. ~~**+ (16/8) las PIRUETAS de actor**~~ → **HECHO fuera de acá (22/8): [PLAN_MANIOBRAS_FASES](PLAN_MANIOBRAS_FASES.md) M1.** `systems/moves.js` dejó de estar casado con `plane`/`run` (recibe un CUERPO y un ESTADO) y `systems/wingmv.js` mete un Fiel que entra de costado, vuela una maniobra del catálogo y se va — probable con `__mvactor('barrel','izq')` y cubierto por `npm run maniobras`. **Cuando C1 se haga, `wingmv` se vuelve su verbo `actor`+`move`: migración, no reescritura** | `check` verde; el relevo se ve igual y vive en data; un actor ejecuta una pirueta del catálogo por timeline |
| **C2 · El estudio 3D** | el escenario 3D: `three-arena.js` como set — cámara programable (`orbit`, `dolly`, `flyby`, `crane`, `cabina`, **`planeador`** — alta y atrás con el mundo ancho abajo, el plano de GliderVR pedido por Matías para piruetas y sacrificios, ver ANALISIS_REFERENTES_3D §P7) y aviones como billboards con las hojas horneadas (elige el frame por ángulo cámara-avión). Sonda `?cine=<id>` | un flyby al buque con dos Fieles pasando, capturado |
