...
| **El final filmado** | una bomba ARMADA que revienta en el casco dispara el **MOMENTUM OBLIGADO**: el mundo a 1/3 (con el marco del MOMENTUM) hasta el cruce, el cuadro se funde a negro y el negro se **sostiene 2,4 s con Puma encima** (la radio va sobre el negro). Recién después: recuento, vuelta u otra pasada. En el negro el avión tiene piso: no se puede ir al agua sin ver | `BL.LENTO`, `FUNDIDO_T`, `NEGRO_T`, `SALIDA_T` |
| **El salto** (lo hace el jugador, 23/9) | como en Malvinas: después de soltar, un palancazo para pasar por encima del buque. Puma grita "¡Arriba, por encima de los palos!" y el altímetro marca en **amarillo** la altura del buque debajo de tu línea. Cruzar por debajo de la silueta = **te llevás los palos**: golpe de chapa (`DMG.death_palos` 45, dos y te caés); si el avión ya venía roto, se cae y entra el relevo. La cámara lenta del impacto es lo que da tiempo a tirar del morro | `step()`, `core/damage.js` |
