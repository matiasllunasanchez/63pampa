...
> **fixtures npm** (`story cuaderno pasada pulso caza persec romper agua tierra chancha rasante misiones tramos charlas`),
...
| **Diálogos EN VUELO** (las charlas de los tramos, sin volar la misión) | MENÚ → MISIONES → **[H]** hasta `DIÁLOGOS EN VUELO` → ENTER · cualquier tecla pasa al siguiente, ESC sale · sondas `__radiodbg` / `__toastbanda` / `__logdbg` / `__decir(txt)` | 20 s |
