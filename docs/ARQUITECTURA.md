...
| `damage.js` | **pura**: los tres MODELOS DE SALUD (`squad` / `integ` / `visual`), la tabla de daño por causa, qué mata siempre y los escalones de avería. La regla: **te disparan → daño; chocás algo → muerte**. Ver [SPEC_AVERIAS.md](sistemas/SPEC_AVERIAS.md) |
...
| `ruta.js` | **pura**: LA RUTA EN KM de una misión (`ruta:`) — la ancla a los bordes de las fases (`anclas`), convierte fracción ↔ km (`posKm`, `fraccionDeKm`, `kmPorMetro`), ubica el horizonte de radar (`lineasRadar`, `techoAlcance`) y valida la data (`validarRuta`). Ver [SPEC_NAFTA_ALCANCE](sistemas/SPEC_NAFTA_ALCANCE.md) |
| `nafta.js` | **pura**: LA NAFTA COMO ALCANCE — el tanque en km según la carga (`tanqueInicial`, `capacidadKm`), gastar/cargar (externos primero), las tres zonas de gasto, el arrastre, el turbo proporcional (`fVelocidad`, `extraTurboPorSeg`), el bingo, soltar tanques (`soltar`, `proximoPilon`) y los tanques como arma (`golpeTanque`, `buqueTanque`) |
...
| `ruta.js` | el estado de LA RUTA en la corrida: la ruta anclada a sus fases, los km recorridos (`pos`, `alBlanco`, `aCasa`, `kmPorM`), si el radar alcanza (`enAlcance`) y el techo corregido (`techo`, que aplica `techoRadar()` de `fases.js`) | — |
| `nafta.js` | el TANQUE DE LA CORRIDA con ruta (`run.tanque`, `run.naftaCap`): lo llena según la carga, traslada a km lo que otros le hacen a `run.fuel` (Chancha, piruetas, golpes), cobra el vuelo por km, suelta tanques y dice cuánto corre el avión según lo que cuelga | `'seco'` |
...
| `soldiers.js` | hoja **horneada** de los soldados (correr × 6 / cuerpo a tierra, × guarnición y desembarco) |
...
| **que en una misión no se pueda morir** (el tutorial) | el campo `sinMuerte: { golpe, piso, gracia }` de `data/missions.js`. Lo lee `onDeath` en `game.js`, que es la única puerta de todas las muertes: el golpe baja la chapa hasta el piso y no releva. Si la muerte era perder o chocar al líder de la persecución, el líder se rearma |
| **los avisos de radio por evento** (el golpe contra el agua, el radar, la media vuelta) | el campo `avisos` de la misión (`{ agua, choque, piso, radar, vuelta, chancha, cada }`) apunta a escenas `tipo: 'AVISO'` de `data/story.js`: un BANCO del que se dice una línea al azar sin repetir la anterior. Los que se repiten respetan `cada`; los de una vez, no. `condorAlcance` saca las líneas de Cóndor lejos de la base |
| **el radar sin barra, solo con voces** | `radar: 'voz'` en el `cfg` de la misión (default `'normal'`). `systems/flight.js` no carga ni dispara, solo anota `run.radarVisto`; `game.js` no dibuja la red y dice el aviso `radar` |
| **un tutorial con pausa, foco y teclas** | el campo `lecciones` de la misión: `{ en, dice, foco, teclas, pausa }`. `dice` es una escena `tipo: 'LECCION'` de una línea; `foco` son nombres de `zonaHud()` (`render/hud.js`) o `'seguir'`; `teclas` son filas `ctrl<Nombre>` de `data/strings.js`, que el juego muestra en su versión de teclado o de mando. Ningún personaje nombra una tecla. `en: 'aterrizaje'` = al entrar a la aproximación final. La pausa es la PAUSA DE DIÁLOGO de siempre (`dlgPausa`) |
| **el cartel del despegue** (de dónde salen y hacia qué) | el campo `despegue: { desde, rumbo }` de cada misión en `data/missions.js`; lo dibuja `drawTakeoff` en `render/hud.js`. Fuera de una misión queda el genérico de `strings.js` (`takeoffTitle`/`takeoffHeading`) |
...
| **LA NAFTA COMO ALCANCE** (ruta en km, radar con alcance, tanque en km, zonas de gasto, turbo proporcional, soltar tanques, el hangar) | todo es **dato de la misión** (`ruta:`, `fases:`, `chanchaVeces`) + las perillas de `data/tuning.js` (bloque "LA NAFTA COMO ALCANCE" y `TQ_*`). La cuenta, pura: `core/ruta.js` y `core/nafta.js`; el estado: `systems/ruta.js` y `systems/nafta.js`; el cobro, en `flightSystem`; el HUD, en `render/hud.js`; el hangar (`'carga'`), en `game.js` + `render/menus.js`. **Sin `ruta`, nada de esto corre** (salvo el turbo proporcional). Sondas `__nafta`, `__chadbg`. Todo lo vigente en [SPEC_NAFTA_ALCANCE.md](sistemas/SPEC_NAFTA_ALCANCE.md) |
| **LA CHANCHA (el reabastecedor, tecla 3)** | `systems/chancha.js` (barra, gates, cita, conexión — devuelve señales y el % de tanque; no toca stores) + `render/chancha.js` (el Hércules procedural, la manguera y la canasta) + las perillas `CH_*` de `data/tuning.js`. El pedido y la radio están en `pedirChancha()`/`chanchaRadio()` de `game.js`; la barra, en `render/hud.js`; la disponibilidad por misión, en el campo `chancha` de `data/missions.js`. **La canasta va a la profundidad de juego (`PZ`)**: a otra profundidad, la caja que se dibuja no es donde hay que poner el avión. Probalo con `npm run chancha`; el spec y las divergencias, en [SPEC_PODER_CHANCHA.md](sistemas/SPEC_PODER_CHANCHA.md). **Con ruta cambian las reglas** (se llama por tramo, la de la ida sin barra, el enganche con naranja → verde, la reserva de ella): ver [SPEC_NAFTA_ALCANCE §5](sistemas/SPEC_NAFTA_ALCANCE.md) |
...
| **cambiar los SOLDADOS** | también son un **asset horneado** (PLAN_HORNEADO B7): rig low-poly en `tools/models/soldiers.js` → `npm run soldados` → `assets/world/soldats/soldados.png` → `render/soldiers.js`. **LA HOJA NO LLEVA CAJAS Y NO HAY QUE MEDIR NADA**: todas las poses salen de la misma cámara, así que la celda es una ventana fija al mundo (`WU` unidades de lado) con el suelo siempre en la misma fila, y dibujar es una sola cuenta. Los dos números del encuadre (`WU`, `SUELO`) están declarados **en el horneador y en el render**, y `npm run unit` los compara: si divergen, el soldado flota o se entierra y no hay error de runtime que lo delate. El soldado se hornea con un **filo de dos tonos** (`BAKE.contorno`) porque sin él desaparece sobre la turba, que es de su mismo verde — ver PLAN_HORNEADO §7.29 |
| **cambiar LA CHANCHA** (el KC-130) | modelo en `tools/models/enemies.js` (`modelHercules`) → `npm run` del horno de enemigos → `assets/world/enemies/chancha.png` → `render/chancha.js`. **SE HORNEA DESDE ABAJO** (`elev: 12` en `tools/bake_enemies.html`): al mismo nivel la silueta es una cruz (de popa pura las cuatro superficies grandes se ven de canto), y a los 31° que pide la geometría de la cita el avión se lee **en picada** — un recorte 2D no tiene con qué vender que el torcido sos vos. Los 12° son deliberados y están anotados: no los "corrijas" a la cuenta exacta (PLAN_HORNEADO §8.32-33). Lo que se pinta por CÓDIGO encima —los cuatro discos de hélice y el arranque de la manguera— **no se pone a ojo**: la hoja declara `puntos` (coordenadas del modelo), el horno los proyecta con su propia cámara y escribe el píxel en `src/data/cajas.js`, y `enemyArt.anclaje()` los lleva a pantalla. Mové un motor en el modelo y la hélice se mueve sola |
